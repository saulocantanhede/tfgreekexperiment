...
basedir="/Users/TF/Downloads/input_xml/xml/" 
...
for files in SET:
	inputfile=basedir+"input/"+files
	outputfile=basedir+"output/"+files
	sentencecounter=0
	bookcounter+=1
	print ('trying to open: '+files)
	#print ('output file:'+files)
	try: 
	...
	while True:
		...
		for line in lines:
			...
			if 'milestone ' in line:
				final=re.sub(r'<mile.*</p>','', line)
				final=re.sub(r'<mile.*stone>','', final)
